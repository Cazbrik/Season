...
/* Test test_null_parse() operation */

TEST(test_null_parse, {

    char *str = "null";
    assert_int(JSON_NULL, null_parse(&str));

    str = "nul";
    assert_int(JSON_INVALID, null_parse(&str));
...
/* Test number_parse() operation */

TEST(test_number_parse, {

    double buf;
    char *str = "-45";
    assert_int(JSON_NUMBER, number_parse(&str, &buf));
...
/* Test string_parse() operation */

TEST(test_string_parse, {

    char *buf;
    char *str = "\"String\"";
    assert_int(JSON_STRING, string_parse(&str, &buf));
    assert_string("String", buf);
    assert_int(0, *str);
    free(buf);

    str = "\"String";
    assert_int(JSON_INVALID, string_parse(&str, &buf));
...
/* Test object_parse() operation */

TEST(test_object_parse, {
    
    struct json_object *buf;
    char *str = "{}";
    assert_int(JSON_OBJECT, object_parse(&str, &buf));
    ...
    str = "{ \"key\" : 23 \"key2\" : \"String\"}";
    assert_int(JSON_INVALID, object_parse(&str, &buf));
